...
#sns.regplot(x = '', y = '',data = da, fit_reg = False, scatter_kws={'alpha':0.2})
...
#sns.jointplot(x = '', y = '',kind = 'kde',data = da).annotate(stats.pearsonr)
...
db.groupby(["RIAGENDRx", "DMDEDUC2x", "DMDMARTLx"]).size().unstack().fillna(0).apply(lambda x: x/x.sum(),axis = 0)
